...
CSmartLight::CSmartLight() {
    ...
    // default value = white 
    RValueLight = RGB_VALUE_DEFAULT;
    GValueLight = RGB_VALUE_DEFAULT;
    BValueLight = RGB_VALUE_DEFAULT;

    statusStr = "{}";
    resetCmd();
}
/*
We are not controlling  the light through serial communication
...
void CSmartLight::changeColor(int red, int blue, int green){
    RValueLight = red;
    GValueLight = green;
    BValueLight = blue;

    RGB.color(RValueLight,GValueLight, BValueLight);
}
...
void CSmartLight::createColorString(){
    colorLightRGB = String::format("{\"R\":%d,\"G\":%d,\"B\":%d,\"Brightness\":%d}", 
        RValueLight, GValueLight, BValueLight, (int)((double)brightness/RGB_BRIGHTNESS_MAX*100.0) );
}

String CSmartLight::getStatusStr(){
    return statusStr;
}

String CSmartLight::getColor(){
    return colorLightRGB;
}
